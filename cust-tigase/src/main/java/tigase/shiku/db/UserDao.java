...
public class UserDao {
	...
	public static Set<String> shieldUserIds=null;
	...
	public  synchronized void startProducer(){
		try {
			...
			logger.info("UserDao init Producer {}   GroupId {} ",ShikuConfigBean.pushMqAddrVal,ShikuConfigBean.pushMqProdGidVal);

			producer=new DefaultMQProducer(ShikuConfigBean.pushMqProdGidVal,new AclClientRPCHook(new SessionCredentials(ShikuConfigBean.pushMqAccesskeyVal,ShikuConfigBean.pushMqSecretkeyVal)));
	...
	* @param @param conn    xmpp  链接
	 */
	public long handleLogin(final XMPPResourceConnection conn){
		String userIdStr=null;
		Long userId = null;
		try {
		
			userIdStr=conn.getUserId().getLocalpart();
			logger.info("UserDao.handleLogin == userId ====>  {} resource {}  isEncrypted {}",userIdStr,conn.getResource(),conn.isEncrypted());
			...
			if(ShikuConfigBean.OPEN_REDIS_AUTH) {
				String messageKey = RedisService.getInstance().querySessonMessageKey(userId+"", conn.getResource());
				logger.info(" login messageKey {}",messageKey); 
				if(!StringUtil.isEmpty(messageKey))
					conn.putSessionData("messageKey", messageKey);
			}
	...
	* @param @param userIdStr    参数
	 ...
	 * @param content
	 */
	public void saveInterceptKeyWord(long userId,long toUserId,String roomJid,String content){
		DBObject dbObj=new BasicDBObject();
		dbObj.put("sender", userId);
		if(toUserId != 0){
			dbObj.put("receiver", toUserId);
		}
		if(!StringUtil.isEmpty(roomJid)){
			dbObj.put("roomJid", roomJid);
		}
		dbObj.put("content", content);
		dbObj.put("createTime", System.currentTimeMillis());
		
		DBCollection collection=getCollection(INTERCEPTKEYWORD);
	...
	* @param @return    参数
	...
	public DBObject getConfig(){
		...
		keys.put("isSaveSystemMsg", 1);
		DBCollection dbCollection = getCollection(CONFIG);
		DBObject obj = dbCollection.findOne(new BasicDBObject(), keys);
		return obj;
	}

	public  boolean  queryShieldUserIds(int userId){
		DBCollection collection = getCollection(USER);
		BasicDBObject query = new BasicDBObject("_id",userId).append("status",-1);
		return 0<collection.count(query);
	}

	public synchronized Set<String>  queryShieldUserIds(){
		DBCollection collection = getCollection(USER);
		BasicDBObject query = new BasicDBObject("status",-1);
		List list = collection.distinct("_id", query);
