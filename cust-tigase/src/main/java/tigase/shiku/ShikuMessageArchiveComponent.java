...
public class ShikuMessageArchiveComponent extends AbstractMessageReceiver {
	...
	private org.slf4j.Logger log = LoggerFactory.getLogger(ShikuMessageArchiveComponent.class.getName());
	...
	private void storeMessage(Packet packet) {
		...
		if (null==ownerStr) {
			log.debug("Owner attribute missing from packet: {0}",
					packet);
			return;
		}
	...
	private void storeMessageChat(Packet packet,String ownerStr){
		try {
			...
			long senderFromId = getUserId(sender_jid);

			long sender = senderFromId;
			...
			if(null!=body){
				...
				if(sender==receiver){
					 ...
					 * 不是发给自己设备的消息
					 */
					if(!jsonObj.getString("toUserId").equals(jsonObj.getString("fromUserId"))){
						return;
					}
			...
			if(0==contextType)
				...
				//log.info("updateMsgReadStatus ==> "+content);
				//消息已读回执处理
				...
				return;
			else if(99 < contextType && 103 != contextType && 104 != contextType && 113 != contextType && 114 != contextType && 202!=contextType){
				//其他 系统 通知消息,单聊保存音视频挂断和取消的xmpp消息 2019-09-27
				return;
			}else if(isReadDel == true) {
				return;
			}else if(0==ShikuConfigBean.isSaveSystemMsg){
				/**
				 * 系统账号发送消息不保存
				 */
				if(senderFromId<10050&&9!=contextType/100){
					return;
				}
			}
	...
	private void storeMessageGrouChat(Packet packet,String ownerStr){
		 ...
		 */
			...
			*/
				 ...
				 * 系统账号发送消息不保存
				 */
				if(senderFromId<10050){
					return;
				}else if(3==jsonObj.getIntValue("msgType")){
					//压测消息不保存
					return;
				}
			}else if (803 < contextType && 807 > contextType) {
				// 群组中的发送秘钥，重置秘钥不保存
				return;
			}
