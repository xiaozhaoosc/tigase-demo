	...
	public Packet filter(Packet packet) {
		...
		if(filterKeyWords(content)){
			...
			if(type == StanzaType.chat){
				// 单聊
				UserDao.getInstance().saveInterceptKeyWord(Long.valueOf(bodyObj.getString("fromUserId")), Long.valueOf(bodyObj.getString("toUserId")), null, content);
			}else if(type == StanzaType.groupchat){
				// 群聊
				UserDao.getInstance().saveInterceptKeyWord(Long.valueOf(bodyObj.getString("fromUserId")), 0, bodyObj.getString("toUserId"), content);
			}
			return null;
		}else{
			/*if(ShikuConfigBean.isDeBugMode()) {
